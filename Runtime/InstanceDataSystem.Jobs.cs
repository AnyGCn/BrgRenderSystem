...
namespace BrgRenderSystem
{
    internal partial class InstanceDataSystem : IDisposable
    {
        ...
        private unsafe struct QueryRendererGroupInstancesMultiJob : IJobParallelForBatch
        {
            ...
            [ReadOnly] public NativeParallelMultiHashMap<int, InstanceHandle> rendererGroupInstanceMultiHash;
            [NativeDisableContainerSafetyRestriction, NoAlias][ReadOnly] public NativeArray<int>.ReadOnly rendererGroupIDs;
            [NativeDisableContainerSafetyRestriction, NoAlias][ReadOnly] public NativeArray<int>.ReadOnly instancesOffsets;
            [NativeDisableContainerSafetyRestriction, NoAlias][ReadOnly] public NativeArray<int>.ReadOnly instancesCounts;
        ...
        private unsafe struct TransformUpdateJob : IJobParallelFor
        {
            ...
            [ReadOnly] public NativeArray<Matrix4x4>.ReadOnly localToWorldMatrices;
            ...
            public void Execute(int index)
            {
                ...
                instanceData.localToWorlds[instanceIndex] = PackedMatrix.FromFloat4x4(l2w);
                instanceData.worldToLocals[instanceIndex] = PackedMatrix.FromFloat4x4(math.inverse(l2w));
        ...
        private struct ReallocateInstancesJob : IJob
        {
            ...
            [ReadOnly] public NativeArray<GPUDrivenPackedRendererData>.ReadOnly packedRendererData;
            // [ReadOnly] public NativeArray<int>.ReadOnly instanceOffsets;
            // [ReadOnly] public NativeArray<int>.ReadOnly instanceCounts;
        ...
        private unsafe struct UpdateRendererInstancesJob : IJobParallelFor
        {
            ...
            public void Execute(int index)
            {
                ...
                var renderingLayerMask = rendererData.renderingLayerMask[index];
                ...
                var instanceFlags = new InstanceFlags(packedRendererData);
                ...
                // if (packedRendererData.staticShadowCaster)
                //     instanceFlags |= InstanceFlags.IsStaticShadow;
                //
                ...
                // if (instancesCount > 0)
                {
                    ...
                    var materialIDs = new SmallIntegerArray(materialCount);
                    ...
                    sharedInstanceData.Set(sharedInstance, rendererGroupID, materialIDs, meshID, localAABB, transformUpdateFlags, instanceFlags, gameObjectLayer, renderingLayerMask,
        ...
        [BurstCompile(DisableSafetyChecks = true, OptimizeFor = OptimizeFor.Performance)]
        private struct PrepareRebuildGPUInstanceIndexJob : IJob
        {
            public NativeList<int> rangesList;
            [NativeDisableParallelForRestriction] public CPUInstanceData instanceData;
            [ReadOnly] public CPUSharedInstanceData sharedInstanceData;
            ...
            public void Execute()
            {
                ...
                for (int i = 0; i < instanceData.handlesLength; ++i)
                {
                    ...
                    int batchHash = HashCode.Combine(sharedInstanceData.meshIDs[sharedIndex], sharedInstanceData.materialIDArrays[sharedIndex]);
                    if (!batchHashMap.TryGetValue(batchHash, out var index))
                    {
                        index = rangesList.Length;
                        batchHashMap[batchHash] = index;
                        rangesList.Add(0);
                    }
                    
                    instanceData.gpuInstances[i] = new GPUInstanceIndex(rangesList[index]++, index);
                }
                ...
                {
                    offset += rangesList[i];
                    rangesList[i] = offset - rangesList[i];
                }
            }
        }
        
        [BurstCompile(DisableSafetyChecks = true, OptimizeFor = OptimizeFor.Performance)]
        private struct UpdateGPUInstanceIndexAndData : IJobParallelFor
        {
            public const int k_BatchSize = 128;

            [NativeDisableParallelForRestriction] public CPUInstanceData instanceData;
            [ReadOnly] public NativeList<int> rangesList;

            public int byteSizePerWindow;
            public int instancePerWindow;
            public int localToWorldMatrixOffset;
            public int worldToLocalMatrixOffset;
            [NativeDisableParallelForRestriction] public NativeArray<float4> outputBuffer;
            
            public unsafe void Execute(int index)
            {
                ref GPUInstanceIndex gpuInstanceIndex = ref UnsafeUtility.ArrayElementAsRef<GPUInstanceIndex>(instanceData.gpuInstances.GetUnsafePtr(), index);
                gpuInstanceIndex = GPUInstanceDataBuffer.IndexToInstance(instancePerWindow, rangesList[gpuInstanceIndex.windowIndex] + gpuInstanceIndex.index);
                byte* gpuAddress = (byte*)outputBuffer.GetUnsafePtr() + gpuInstanceIndex.index * UnsafeUtility.SizeOf<PackedMatrix>() + gpuInstanceIndex.windowIndex * byteSizePerWindow;
                UnsafeUtility.CopyStructureToPtr(ref UnsafeUtility.ArrayElementAsRef<PackedMatrix>(instanceData.localToWorlds.GetUnsafeReadOnlyPtr(), index), gpuAddress + localToWorldMatrixOffset);
                UnsafeUtility.CopyStructureToPtr(ref UnsafeUtility.ArrayElementAsRef<PackedMatrix>(instanceData.worldToLocals.GetUnsafeReadOnlyPtr(), index), gpuAddress + worldToLocalMatrixOffset);
            }
        }
        
        [BurstCompile(DisableSafetyChecks = true, OptimizeFor = OptimizeFor.Performance)]
        private struct UpdateGPUInstanceData : IJobParallelFor
        {
            public const int k_BatchSize = 128;

            [ReadOnly] public NativeArray<InstanceHandle> instances;
            [ReadOnly, NativeDisableParallelForRestriction] public CPUInstanceData instanceData;
