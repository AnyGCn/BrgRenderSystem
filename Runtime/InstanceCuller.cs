...
namespace BrgRenderSystem
{
    ...
    internal struct CullingJob : IJobParallelFor
    {
        ...
        unsafe float CalculateLODVisibility(int instanceIndex, int sharedInstanceIndex, InstanceFlags instanceFlags)
        {
            ...
            var lodDataIndexAndMask = instanceData.lodGroupAndMasks[instanceIndex];
            ...
            else if(viewType < BatchCullingViewType.SelectionOutline && instanceFlags.smallMeshCulling)
            {
                ref readonly AABB worldAABB = ref UnsafeUtility.ArrayElementAsRef<AABB>(instanceData.worldAABBs.GetUnsafeReadOnlyPtr(), instanceIndex);
        ...
        private unsafe uint CalculateVisibilityMask(int instanceIndex, int sharedInstanceIndex, InstanceFlags instanceFlags)
        {
            ...
            if (cullLightmappedShadowCasters && instanceFlags.affectsLightmaps)
                return 0;
            ...
            if (viewType == BatchCullingViewType.Camera && instanceFlags.shadowCastingMode == ShadowCastingMode.ShadowsOnly)
                return 0;
            if (viewType == BatchCullingViewType.Light && instanceFlags.shadowCastingMode == ShadowCastingMode.Off)
                return 0;
    ...
    internal unsafe struct AllocateBinsPerBatch : IJobParallelFor
    {
        ...
        unsafe public void Execute(int batchIndex)
        {
            ...
            if (binCount > 0)
            {
                ...
                int shiftForVisibilityMask = 1 + (binningConfig.supportsCrossFade ? 1 : 0);
        ...
        unsafe public void Execute()
        {
            ...
            for (;;)
            {
                ...
                for (int rangeIndex = 0; rangeIndex < drawRanges.Length; ++rangeIndex)
                {
                    ...
                    int rangeDrawCommandOffset = false ? outIndirectCommandIndex : outDirectCommandIndex;
                    ...
                    if (rangeDrawCommandCount != 0)
                    {
                        ...
                        output.drawRanges[outRangeIndex] = new BatchDrawRange
                        {
                            ...
                            filterSettings = new BatchFilterSettings
                            {
                                ...
                                motionMode = MotionVectorGenerationMode.Camera, // rangeKey.motionMode,
    ...
    internal unsafe struct DrawCommandOutputPerBatch : IJobParallelFor
    {
        ...
        unsafe int EncodeGPUInstanceIndexAndCrossFade(int rendererIndex, bool negateCrossFade)
        {
            ...
            gpuInstanceIndex |= crossFadeValue << 24;
            return gpuInstanceIndex;
        }
...
//                 else
                {
                    ...
                    output.drawCommands[drawCommandOffset] = new BatchDrawCommand
                    {
                        ...
                        batchID = batchIDs[drawBatch.key.windowID],
                        materialID = drawBatch.key.materialID,
                        splitVisibilityMask = (ushort)visibilityMask,
						// lightmapIndex = (ushort)drawBatch.key.lightmapIndex,
                        sortingPosition = sortingPosition,
    ...
    internal struct InstanceCuller : IDisposable
    {
            ...
            OcclusionCullingCommon occlusionCullingCommon)
        {
            ...
            var binningConfig = new BinningConfig
            {
                ...
                // supportsMotionCheck = false, // TODO: could disable here if RP never needs object motion vectors, for now always batch on it
            };
            ...
            if (cc.viewType == BatchCullingViewType.Picking)
            {
                ...
                cullingJobHandle = CreatePickingCullingOutputJob_EditorOnly(cc, cullingOutput, instanceData, sharedInstanceData,
        ...
        private JobHandle CreatePickingCullingOutputJob_EditorOnly(in BatchCullingContext cc, BatchCullingOutput cullingOutput,
            in CPUInstanceData.ReadOnly instanceData, in CPUSharedInstanceData.ReadOnly sharedInstanceData,
            in CPUDrawInstanceData drawInstanceData, NativeArray<BatchID> batchIDs, NativeArray<byte> rendererVisibilityMasks,
